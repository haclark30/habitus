...
type HabitService interface {
	CountUp(habitId int, habitLogId int) services.HabitAndLog
	CountDown(habitId int, habitLogId int) services.HabitAndLog
	AddHabit(userId int, habitName string, hasUp, hasDown bool) services.HabitAndLog
	GetHabits(userId int, datetime int64) []services.HabitAndLog
}
...
func (h *HabitHandler) CountUp(w http.ResponseWriter, r *http.Request) {
	...
	habit := h.HabitService.CountUp(habitIndex, habitLogId)
...
func (h *HabitHandler) CountDown(w http.ResponseWriter, r *http.Request) {
	habitIndex, _ := strconv.Atoi(chi.URLParam(r, "habitId"))
	habitLogId, _ := strconv.Atoi(chi.URLParam(r, "habitLogId"))
	habit := h.HabitService.CountDown(habitIndex, habitLogId)
	components.Habit(habit).Render(r.Context(), w)
}
